...
func RegisterHTTP(rg *gin.RouterGroup, svc *Service) {
	rg.GET("/stores/:storeId/summary", func(c *gin.Context) {
		...
		summary, err := svc.GetDailySummary(merchantID, storeID, day)
		if err != nil {
	...
	rg.GET("/stores/:storeId/exceptions", func(c *gin.Context) {
		rawMerchantID, ok := c.Get(auth.ContextMerchantIDKey)
		if !ok {
			c.JSON(http.StatusUnauthorized, gin.H{"error": "missing merchant context"})
			return
		...
		exceptions, err := svc.ListExceptions(merchantID, storeID, day)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
